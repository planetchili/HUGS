...
template <typename T>
inline bool CornerIsConvex( _Vec2<T> v0,_Vec2<T> v1,_Vec2<T> v2 )
{
	return ( v1 - v0 ).CrossWith( v2 - v1 ) <= ( T )0.0;
}

template <typename T>
bool TriangleContainsPoint( _Vec2<T> v0,_Vec2<T> v1,_Vec2<T> v2,_Vec2<T> p )
{
	auto IsSameSide = [p]( _Vec2<T> a,_Vec2<T> b,_Vec2<T> c ) -> bool
	{
		const _Vec2<T> ab = b - a;
		return ab.CrossWith( p - a ) * ab.CrossWith( c - a ) >= ( T )0.0;
	};
